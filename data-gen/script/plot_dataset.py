...
def plot_dataset(csv_path, output_html=None, output_png=None, plot_limit=100000):
    ...
    plot_classes_text = [
        f"Anomaly: {class_names[lbl].capitalize()}" if lbl > 0 and lbl in class_names else (f"Anomaly Class {lbl}" if lbl > 0 else "Normal Background") 
        for lbl in is_anomaly
    ]
    ...
    if len(anomalous_points) > 0:
        ...
        fig.add_trace(go.Scatter(
            x=time_step[anomalous_points],
            y=gamma_dose[anomalous_points],
            mode='markers',
            name='Anomalies',
