...
CONFIG = {
    ...
    "VARIANCE_MAX":       0.10,         # Max shape variance (distortion applied to the template)
...
def create_synthetic_dataset(
    ...
    std_noise = CONFIG["BACKGROUND_STD"] * noise_multiplier
    ...
    # 3. Configuration
    N_synthetic = total_points if total_points is not None else CONFIG["TOTAL_POINTS"]
    CHUNK_SIZE = CONFIG["CHUNK_SIZE"] if not return_df else N_synthetic  # Do it in one chunk if in-memory
    ...
    for idx in inject_points:
        ...
        variance_level = variance_override if variance_override is not None else np.random.uniform(CONFIG["VARIANCE_MIN"], CONFIG["VARIANCE_MAX"])
    ...
    plot_time, plot_gamma, plot_anomaly_vals, plot_labels, plot_classes_text, plot_baseline = [], [], [], [], [], []
    ...
    try:
        ...
        for chunk_idx in range(num_chunks):
            ...
            if return_df:
                all_time.extend(range(chunk_start, chunk_end))
                all_gamma.extend(chunk_background[:chunk_len].tolist())
                all_labels.extend(chunk_labels[:chunk_len].tolist())
            elif csv_file:
                for i in range(chunk_len):
                    global_t = chunk_start + i
                    csv_file.write(f"{global_t},{chunk_background[i]},{chunk_labels[i]}\n")
            
            if not return_df and chunk_start < plot_limit:
    ...
    if return_df:
        return pd.DataFrame({'time_step': all_time, 'gamma_dose': all_gamma, 'is_anomaly': all_labels})
    ...
    ax.plot(plot_time, plot_gamma, color='#1f77b4', linewidth=1, label='Simulated Gamma Noise')

    # Removed the red scatter dots from the PNG as requested
    anomalous_points = np.where(plot_labels > 0)[0]
    
    ax.plot(plot_time, plot_anomaly_vals + global_mean, color='orange', linewidth=2, label='Pure Anomaly Shapes (No Noise)')
    ax.plot(plot_time, plot_baseline, color='yellow', linewidth=2, linestyle='--', label='Wandering Baseline')

    ax.set_title('SYNTHETIC DATASET: Simulated Sensor Noise + Custom Anomaly Forms', fontsize=14)
    ax.set_xlabel('Time Steps')
    ax.set_ylabel('Simulated Gamma Dose')
    ax.legend(loc='upper right')
    
    # Limit x-axis to first 3 anomalies for the static PNG snapshot
    is_anom_bool = plot_labels > 0
    ...
    if len(starts) >= 3:
        padding = 2000
        xlim_start = plot_time[max(0, starts[0] - padding)]
        xlim_end = plot_time[min(len(plot_time)-1, ends[2] + padding)]
        ax.set_xlim(xlim_start, xlim_end)
    elif len(starts) > 0:
        padding = 2000
        xlim_start = plot_time[max(0, starts[0] - padding)]
        xlim_end = plot_time[min(len(plot_time)-1, ends[-1] + padding)]
        ax.set_xlim(xlim_start, xlim_end)
    ...
    # 6. Plotly HTML Preview
    fig_plotly = go.Figure()

    fig_plotly.add_trace(go.Scatter(
        x=plot_time, y=plot_gamma, mode='lines', name='Simulated Gamma Noise',
    ...
    fig_plotly.add_trace(go.Scatter(
        x=plot_time[anomalous_points], y=plot_gamma[anomalous_points], mode='markers',
        name='Injected Anomalies', marker=dict(color='red', size=5),
        hovertext=anomalous_texts, hoverinfo="x+y+text"
    ))

    fig_plotly.add_trace(go.Scatter(
        x=plot_time, y=plot_anomaly_vals + global_mean, mode='lines', 
        name='Pure Anomaly Shapes (No Noise)', line=dict(color='orange', width=2), hoverinfo="skip"
    ))

    fig_plotly.add_trace(go.Scatter(
        x=plot_time, y=plot_baseline, mode='lines', 
        name='Wandering Baseline', line=dict(color='yellow', width=2, dash='dash'), hoverinfo="skip"
    ))

    fig_plotly.update_layout(
        title='SYNTHETIC DATASET: Simulated Sensor Noise + Custom Anomaly Forms',
        xaxis_title='Time Steps', yaxis_title='Simulated Gamma Dose',
        hovermode='x unified', template='plotly_white'
    )

    output_plot_path = os.path.join(logs_dir, 'synthetic_custom_dataset.html')
    fig_plotly.write_html(output_plot_path)

    print(f"\nFinal interactive plot saved to: {output_plot_path}")
    print(f"Final dataset CSV saved for training to: {output_csv_path}")
    print(f"Static Plot saved to: {output_png_path}")
