...
architectures of \cref{chap:methods} rest on. The goal is to make the
...
$x_t$ to one of $K$ classes. In our benchmark $K = 7$: six anomaly
templates plus a \texttt{Normal Background} class (\cref{sec:templates}).
...
samples, each window inherits a single label through a
\emph{coverage} rule: if at least a fraction $\tau_{\text{cov}}$ of the
window's samples fall inside an anomaly, the window takes the majority
anomaly class among them; otherwise it is labelled
\texttt{Normal Background} (\cref{fig:windowing} in
\cref{sec:windowing}). We use $\tau_{\text{cov}} = 0.10$. The floor stops
a window from being called anomalous on the strength of a handful of
boundary samples, while staying low enough that the dense injection of
the benchmark (\cref{sec:generator}) still yields many positive windows
of every morphology. The deliberately modest threshold also leaves the
start and end of events slightly fuzzy --- a property that helps the
network generalise to events whose exact extent is uncertain.
...
the total count. The numerical values this rule produces for our seven
classes are reported in \cref{sec:training}; the mismatch between this
training prior and the far sparser anomaly rate a real beacon would see
is exactly the kind of distribution shift we stress-test in
\cref{sec:res-robust}.
...
(\cref{sec:realdata}), feeding the absolute dose to the network is
...
deviations from the local baseline, matching the slowly drifting
baseline characterised in \cref{sec:realdata}.
