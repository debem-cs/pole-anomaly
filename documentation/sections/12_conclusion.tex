...
On the v2 synthetic benchmark the three architectures cluster at the
same event-level macro F1 ($\sim 86\%$), so the synthetic numbers do
not separate them. The separation appears on two other axes:
\begin{itemize}[leftmargin=*, itemsep=0.2em]
    ...
    to a single-class plateau already at $m_{\text{noise}} \approx 2$.
    \item \textbf{Real-data behaviour} (\cref{chap:v2real}): the three
    models look qualitatively similar on the v2 real-data plots, but
    the ResNet is the cleanest --- the v2 ResNet is the one whose
    background confidence stays closest to $100\%$ on the quiet
    stretches without any of the residual jitter the 1D-CNN still
    shows.
\end{itemize}
...
enough to retrain in under ten minutes on a laptop GPU. The
CNN+Attention model is kept in the pipeline as the most promising
candidate for longer-window analyses (\cref{sec:future-attention}).
The baseline 1D-CNN is the fastest to train but its noise fragility
makes it unfit for a beacon that must run across seasons.
...
\begin{itemize}[leftmargin=*, itemsep=0.25em]
    ...
    \item \textbf{Short events.} The v2 event-level extractor loses
    sensitivity on events shorter than $\sim 50$ samples, even though
    the v2 generator is calibrated to produce events as short as $30$.
    This is the four missed events of \cref{tab:v2-event-prf}.
