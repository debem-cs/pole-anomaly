...
$x_t$ to one of $K=7$ classes (six anomaly types plus the normal
background).
...
samples, we follow a simple rule: the window inherits the most frequent
non-zero label inside it if more than $10\%$ of its samples are
anomalous, and is labelled \texttt{Normal Background} otherwise
(\cref{fig:sliding-window}). This makes the boundaries of events
slightly fuzzy --- a deliberate choice that helps the network
generalise to events whose exact start and end are uncertain.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{figures/sliding_window}
    \caption{Sliding-window construction on a fragment of the synthetic
    signal. Each coloured bar represents one $512$-sample training
    window; red bars indicate windows that contain more than $10\%$
    anomaly points, so they inherit the underlying class label.}
...
the total count. In our experiments this gives weights
$\mathbf{w} = (0.46, 1.18, 1.27, 1.32, 1.24, 1.15, 1.31)$ for the seven
classes: the \texttt{Normal} class is down-weighted by about a factor
of two relative to the anomaly classes. We will see in
\cref{chap:realdata} that this choice, harmless on the synthetic test
set, becomes problematic at deployment time because of the dramatic
class prior shift in the real data.
...
deviations from the local baseline --- but it has a hidden cost that
becomes apparent in \cref{chap:realdata}.
