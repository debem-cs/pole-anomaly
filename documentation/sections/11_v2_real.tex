...
\paragraph{The remaining detection floor is the short events.} The
shortest hand-labelled events (June, $32$--$55$ samples) sit at the
borderline of the windowing labelling rule
(\cref{sec:framing}). The v2 generator is calibrated to produce events
this short, but the event-level extractor of \cref{sec:event-eval}
loses sensitivity on them --- the v2 synthetic-test result already
showed this on the four missed events of \cref{tab:v2-event-prf}.
Detecting these shortest events with high recall is the natural next
problem.
