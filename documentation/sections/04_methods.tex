...
\begin{figure}[H]
    ...
    \caption{Sliding-window labelling on a fragment containing one event (orange
    span, top). Each bar (bottom) is one $512$-sample window; a window is
    labelled anomalous (red) when at least $\tau_{\mathrm{cov}}=10\%$ of its
    samples fall inside the event, and \texttt{Normal} (grey) otherwise.}
    \label{fig:windowing}
\end{figure}
