...
\begin{itemize}[leftmargin=*, itemsep=0.2em]
    \item The \textbf{1D-CNN} F1 drops below $50\%$ at $m_{\text{noise}}
    \approx 1.9$ and continues to fall steeply to $\sim 4\%$ at the
    end of the sweep --- the same kind of single-class collapse already
    visible in v1 (\cref{fig:v1-robustness}, left).
    \item The \textbf{ResNet} and \textbf{CNN+Attention} models stay
    above $50\%$ until $m_{\text{noise}} \approx 2.6$ and continue at
    $\sim 40\%$ at $m_{\text{noise}} = 3$, where the noise standard
    deviation is three times the value they were trained on.
\end{itemize}
...
\begin{itemize}[leftmargin=*, itemsep=0.2em]
    ...
    \approx 5.7$; the \textbf{CNN+Attention} matches it.
...
\paragraph{Self-attention does not buy extra robustness here.} On both
axes the CNN+Attention model is essentially tied with the ResNet. The
...
anomalies rarely extend past $\sim 800$ samples, so attention does not
have much extra context to exploit. On a much longer window --- say a
$24$-hour window with several events of interest --- we would expect
the attention model to pull ahead.
...
F1 by $m_{\text{noise}} \approx 1.9$ confirms that the BatchNorm-based
